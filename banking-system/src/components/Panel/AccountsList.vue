<template>
    <v-app style="background-color: #353B51;">
        ...
        <div class="header">
          <h1 style="color:white;">لیست حساب ها</h1>
        </div>
        <div class="card">
          <v-card width="70%">
              <v-card-title>
  ...
  export default {
    ...
    data: () => ({
      ...
      headers: [
        ...
        { text: 'شماره حساب', value: 'c_dp_num' , filterable: true,},
        { text: 'نوع حساب', value: 'c_dp_num__deposit_type'},
      ],
    }),
    ...
    beforeCreate(){
      this.users = [];
      var that = this;
      axios.get('http://127.0.0.1:8000/all_deposits/')
      .then( (response) => {
        const data = response.data;
        that.users = data;
        console.log(that.users);
